...
import (
	...
	"github.com/joshpetersdev/liftledger/internal/utils"
)
...
func (wh *WorkoutHandler) HandleCreateWorkout(w http.ResponseWriter, r *http.Request) {
  ...
  err := json.NewDecoder(r.Body).Decode(&workout)
	if err != nil {
		...
		return // important to have otherwise clients will see the error but continue with function
	}
...
func (wh *WorkoutHandler) HandleUpdateWorkoutByID(w http.ResponseWriter, r *http.Request) {
  workoutID, err := utils.ReadIDParam(r)
	...
	var updateWorkoutRequest struct {
		// Fields use pointers so we can distinguish between:
		// - not provided (nil) → no update
		// - provided but empty/zero ("" or 0) → explicit update to that value
		// Slices don’t need pointers because a missing array in JSON decodes to nil
		// while an empty array decodes to [].
...
func (wh *WorkoutHandler) HandleDeleteWorkoutByID(w http.ResponseWriter, r *http.Request) {
  ...
  err = wh.workoutStore.DeleteWorkout(paramsWorkoutID)
	...
	utils.WriteJSON(w, http.StatusOK, utils.Envelope{"workout": "workout successfully deleted"})
}
